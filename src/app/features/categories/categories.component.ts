...
export class CategoriesComponent implements OnInit {
  ...
  openCreate(): void {
    const ref = this.dialog.open(CategoryFormDialogComponent, { data: null, width: '450px' });
    ref.afterClosed().subscribe(result => { if (result) this.loadData(); });
  }

  openEdit(cat: CategoryDto): void {
    const ref = this.dialog.open(CategoryFormDialogComponent, { data: cat, width: '450px' });
    ref.afterClosed().subscribe(result => { if (result) this.loadData(); });
  }
